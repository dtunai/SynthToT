...
@click.option(
    "--package-name",
    prompt="Python package name (e.g. 'synthtot')",
...
def main(
    ...
    replacements = [
        ...
        ("synthtot", package_actual_name),
        ("synthtot", package_dir_name),
    ]
    if dry_run:
    ...
    # Rename 'synthtot' directory to `package_dir_name`.
    if not dry_run:
        (REPO_BASE / "synthtot").replace(REPO_BASE / package_dir_name)
    else:
        print(f"Renaming 'synthtot' directory to '{package_dir_name}'")
