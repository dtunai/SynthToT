...
def process_input_data(
    chain: SequentialChain, input_data: List[Dict[str, Any]], prompt_template
) -> List[Dict[str, Any]]:
    """
...
def entrypoint(
    ...
    chain_creator = ChainGenerator(max_tokens=max_tokens, model=model, temperature=temperature)
    all_chains = chain_creator.create_all_chains()
    overall_chain = create_sequential_chain(all_chains)
    all_outputs = process_input_data(overall_chain, input_data, prompt_template)
